...
/*	-------------------------------------------
	There is a issue of sd card mount fail "Damaged USB stroage"
	- Cause
		Page allocate Fail for sda block driver in the kernel When insert sd card using gender.
	- Solution
		Insert Some code in the below function
		.  
			1. static struct parsed_partitions *check_partition(struct gendisk *hd, struct block_device *bdev);
			2. int rescan_partitions(struct gendisk *disk, struct block_device *bdev);

		Page allocate once, when insert sd carf first time.
...
check_partition(struct gendisk *hd, struct block_device *bdev)
{
	...
	int is_sda = strcmp(bdev->bd_disk->disk_name, "sda");
	if(is_sda || !g_state)
	{
		for(retry = 0 ; retry < 20 ; retry ++)
		{
			...
			schedule();
			printk(KERN_INFO "%s : Fail kzalloc fail(%d/100) %s\n",__func__, retry, bdev->bd_disk->disk_name);
		}
...
rescan:
	if (state && !IS_ERR(state)) {
		...
		if (!strcmp(bdev->bd_disk->disk_name,"sda"))
			g_state = NULL;
	}
