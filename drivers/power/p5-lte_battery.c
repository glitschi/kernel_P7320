...
#define P5LTE_MONITOR_WORK
//#define P5LTE_CUR_AVG_WORK /* for test */
#endif
...
#if defined(P5LTE_MONITOR_WORK) || defined(P5LTE_CUR_AVG_WORK)
	struct workqueue_struct *monitor_workqueue;	/* Current Average Work Queue */
#endif
#ifdef P5LTE_MONITOR_WORK
...
/* TBD : Preventing soc drop in case of full -> discharge transition at the lower full voltage */
#if defined(CONFIG_TARGET_LOCALE_KOR_SKT) || defined(CONFIG_TARGET_LOCALE_KOR_KT) || defined(CONFIG_TARGET_LOCALE_KOR_LGU)
	if (fg_vcell > battery->pdata->recharge_voltage) {
			...
			char *buf)
{
	...
	case BATT_FG_RAW_SOC:
		...
		/* for fg test */
		/*
		if (!test_batterydata->info.batt_is_full &&
			test_batterydata->info.charging_enabled &&
			temp_soc == 100) {
			temp_soc = 99;
		}
		if ((max17042_chip_data->info.psoc > 120) &&
			temp_soc == 1) {
			temp_soc = 2;
		}
		*/
