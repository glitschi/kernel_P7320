...
extern unsigned long get_input_editor_state(void);
							...
							unsigned int new_level)
{
	...
	if ( !strcmp(device->name, "kgsl-3d0")) {
		if ((get_input_editor_state()==1) && (new_level==0)) {
			pwr->pwrlevels[0].gpu_freq = KGSL_200MHZ_FREQ;
			pwr->pwrlevels[0].bus_freq = 1;
		} else {
			pwr->pwrlevels[0].gpu_freq = KGSL_266MHZ_FREQ;
