...
#if defined(CONFIG_MACH_P8_LTE) //kks_111030 beacause of amp off while speaker call / Kenel suspend
...
void wm8994_set_normal_speaker(int onoff) //kks_111017
{
	...
	if(onoff==1){
		...
		wm8994_reg_write(amp, 0x26, 0x017d); //speaker volume left //kks_111018 0x017F->0x017d
		wm8994_reg_write(amp, 0x27, 0x017d); //speaker volume right //kks_111018 0x017F->0x017d
...
void wm8994_set_speaker(int onoff)
{
	...
	if(onoff==1){
		...
		// Analogue out gain control
		wm8994_reg_write(amp, 0x25, 0x0164); //kks_111021 0x017F->0x0164 //kks_111018 0x016D->0x017F
...
void wm8994_set_speaker_headset(int onoff)
{
	...
	if(onoff==1){
		...
		wm8994_reg_write(amp, 0x26, 0x017f); //speaker volume left  //kks_111021 0x0179->0x017f //kks_111018 0x017C->0x0179
		wm8994_reg_write(amp, 0x27, 0x017f); //speaker volume right   //kks_111021 0x0179->0x017f //kks_111018 0x017C->0x0179
		...
		wm8994_reg_write(amp, 0x1C, 0x015b); //kks_111026 0x005b->0x015b //kks_111021 0x0167->0x005b
		wm8994_reg_write(amp, 0x1D, 0x015b); //kks_111026 0x005b->0x015b //kks_111021 0x0167->0x005b
...
#if defined(CONFIG_MACH_P8_LTE) //kks_111030
	/* wake lock init */
...
#ifdef CONFIG_MACH_P4_LTE
// SPK_SEL
	rc = pm8058_gpio_config(PMIC_GPIO_SPK_SEL, &spk_sel);
	if (rc) {
		pr_err("%s PMIC_GPIO_AMP_EN config failed\n", __func__);
		return rc;
	}
