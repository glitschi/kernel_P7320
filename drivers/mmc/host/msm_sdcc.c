...
static void msmsdcc_reset_and_restore(struct msmsdcc_host *host)
{
	...
	if (host->plat->sdcc_v4_sup) {
		...
		//ADD_START 
		if(host->plat->is_sdio_al_client && host->mmc->card) 
		{ 
		int i; 
		...
		if (ret)
			pr_err("%s: Failed to set clk rate %u Hz. err %d\n",
					...
					host->clk_rate, ret);
		dsb();
		pr_info("%s: %s pwr: %d CLK:%d MCIPWR:0x%08lx\n", mmc_hostname(host->mmc), __func__, host->pwr, host->clks_on, readl_relaxed(host->base + MMCIPOWER));
	}
...
msmsdcc_stop_data(struct msmsdcc_host *host)
{
	...
	//ADD_START
	if(readl_relaxed(host->base + MMCIDATACTRL) & MCI_DPSM_ENABLE)
	{ 
		int i; 
		pr_info("%s: ====SDCC registers=== in %s MCI_DPSM_EN is  not cleared!\n", mmc_hostname(host->mmc), __func__); 
...
static void msmsdcc_dump_regs(struct msmsdcc_host *host)
{
	u32 i;

	for (i = 0; i <= MMCIFIFOCNT; i += 4)
		pr_err("%p: %08x\n", host->base + i, readl_relaxed(host->base + i));
}
...
msmsdcc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (host->pwr != pwr) {
		...
		WARN_ON(!pwr && ios->clock);
		dsb();
		pr_info("%s: %s pwr: %d MCIPWR:0x%08lx \n", mmc_hostname(host->mmc), __func__, pwr, readl_relaxed(host->base + MMCIPOWER));
	}
	if (!host->clks_on) {
...
#ifndef CONFIG_ATHEROS_WIFI   // #ifdef BROADCOM_WIFI // for broadcomm wifi   
	if(host->pdev_id == 4)   // sdc4
	{
