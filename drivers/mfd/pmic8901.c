...
ssbi_write(struct i2c_client *client, u16 addr, const u8 *buf, size_t len)
{
	...
	if (rc != 1) {
		pr_err("%s: ERROR: PM8901 ssbi write failed, rc=%d\n",
			__func__, rc);
		dump_ssbi_registers();
	}
	return (rc == 1) ? 0 : rc;
...
ssbi_read(struct i2c_client *client, u16 addr, u8 *buf, size_t len)
{
	...
	rc = i2c_transfer(client->adapter, &msg, 1);
	if (rc != 1) {
		pr_err("%s: ERROR: PM8901 ssbi read failed, rc=%d\n",
			__func__, rc);
		dump_ssbi_registers();
	}
	return (rc == 1) ? 0 : rc;
			...
			const struct i2c_device_id *id)
{
...
#if defined(CONFIG_MACH_P5_LTE) && defined(CONFIG_TARGET_LOCALE_EUR_OPEN)
	...
	if ( system_rev <= 0x07 )
	{
		rc = pm8901_preload_dVdd();
		if (rc < 0)
			pr_err("%s: could not preload dVdd: %d\n", __func__, rc);
	}
#endif
