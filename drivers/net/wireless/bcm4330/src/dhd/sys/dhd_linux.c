...
#ifdef CONFIG_MACH_P5_LTE	/* Power On */
	wifi_set_power(1, 50);	//P5_LTE can't Wi-Fi turning on after Mobile AP off
#else
...
#ifdef CONFIG_MACH_P5_LTE	/* Power Off */
	wifi_set_power(0, 50);	//P5_LTE can't Wi-Fi turning on after Mobile AP off
#else
...
dhd_os_wd_timer(void *bus, uint wdtick)
{
	...
	if (wdtick) {
		...
		if (save_dhd_watchdog_ms != dhd_watchdog_ms) {
			...
			*/
#ifdef CONFIG_ARCH_MSM8X60 //Error dection for timer crash problem in dhd watchdog timer
			init_timer(&dhd->timer);
			dhd->timer.data = (ulong)dhd;
			dhd->timer.function = dhd_watchdog;
#endif
