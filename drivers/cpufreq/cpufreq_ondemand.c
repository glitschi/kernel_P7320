...
#define LIMIT_MAX_FREQ_INPUTEDITOR // max freq to 1026000Hz for input editor
...
#ifdef LIMIT_MAX_FREQ_INPUTEDITOR // limit max freq
void set_input_editor_state(bool onOff);
unsigned long get_input_editor_state(void);

static ssize_t show_touch_boost(struct kobject *kobj,
				      struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", get_input_editor_state());
}
#endif
...
#ifdef LIMIT_MAX_FREQ_INPUTEDITOR // limit max freq
static bool input_editor_state = false;

void set_input_editor_state(bool onOff)
{
	if (onOff)
	{
		input_editor_state = false;
		cpufreq_set_limit(INPUTEDITOR_STOP);
	}
	else
	{
		input_editor_state = true;
...
#ifdef LIMIT_MAX_FREQ_INPUTEDITOR
        if (get_input_editor_state() == 1)
            return;
#endif
