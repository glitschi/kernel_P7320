...
#ifdef CAMERA_ESD_TEST_CODE
static int b_esd_detected=false;
static int b_esd_timeout_det=false;
...
static int s5k5ccaf_sensor_burst_write_list(const u32 *list, int size, char *name)
{
	...
	printk("s5k5ccaf_sensor_burst_write_list : %s\n", name);
...
int s5k5ccaf_set_flash(int mode, int onoff)
{
	...
	if(torch_light_on) {
		CAM_DEBUG("can't control Flash in torch mode ON");
		return 0;
	}
...
int s5k5ccaf_set_brightness(int brightness)
{
	...
	switch(brightness) {
	case S5K5CCGX_BR_PLUS_4 :
		CAM_DEBUG("S5K5CCGX_BR_PLUS_4");
		S5K5CCAF_WRITE_LIST(s5k5ccaf_brightness_p_4);
		break;
		
	case S5K5CCGX_BR_PLUS_3 :
		CAM_DEBUG("S5K5CCGX_BR_PLUS_3");
		S5K5CCAF_WRITE_LIST(s5k5ccaf_brightness_p_3);
		break;

	case S5K5CCGX_BR_PLUS_2 :
		CAM_DEBUG("S5K5CCGX_BR_PLUS_2");
		S5K5CCAF_WRITE_LIST(s5k5ccaf_brightness_p_2);
		break;

	case S5K5CCGX_BR_PLUS_1 :
		CAM_DEBUG("S5K5CCGX_BR_PLUS_1");
		S5K5CCAF_WRITE_LIST(s5k5ccaf_brightness_p_1);
		break;

	case S5K5CCGX_BR_DEFAULT :
		CAM_DEBUG("S5K5CCGX_BR_DEFAULT");
		S5K5CCAF_WRITE_LIST(s5k5ccaf_brightness_0);
		break;

	case S5K5CCGX_BR_MINUS_1 :
		CAM_DEBUG("S5K5CCGX_BR_MINUS_1");
		S5K5CCAF_WRITE_LIST(s5k5ccaf_brightness_m_1);
		break;

	case S5K5CCGX_BR_MINUS_2 :
		CAM_DEBUG("S5K5CCGX_BR_MINUS_2");
		S5K5CCAF_WRITE_LIST(s5k5ccaf_brightness_m_2);
		break;

	case S5K5CCGX_BR_MINUS_3 :
		CAM_DEBUG("S5K5CCGX_BR_MINUS_3");
		S5K5CCAF_WRITE_LIST(s5k5ccaf_brightness_m_3);
		break;

	case S5K5CCGX_BR_MINUS_4 :
		CAM_DEBUG("S5K5CCGX_BR_MINUS_4");
...
int s5k5ccaf_set_metering(char metering)
{
	...
	switch(metering) {
	case S5K5CCGX_METERING_NORMAL :
		CAM_DEBUG("S5K5CCGX_METERING_NORMAL");
		S5K5CCAF_WRITE_LIST(s5k5ccaf_metering_normal);
		break;
		
	case S5K5CCGX_METERING_SPOT :
		CAM_DEBUG("S5K5CCGX_METERING_SPOT");
		S5K5CCAF_WRITE_LIST(s5k5ccaf_metering_spot);
		break;

	case S5K5CCGX_METERING_CENTER :
		CAM_DEBUG("S5K5CCGX_METERING_CENTER");
...
{	
	...
	if(lock) {
		if(s5k5ccaf_ctrl->settings.ae_awb_lock == 0) {
			...
			if(s5k5ccaf_ctrl->settings.wb == S5K5CCGX_WB_AUTO) {
	...
	if(status) {	// AF start
		...
		if(s5k5ccaf_ctrl->hd_enabled) {
			...
			if(af_low_lux) {
				...
				msleep(200);	//200ms delay after AF Start(from 5CC guide)
			}
		}
	} else {	// AF stop(abort)
...
int s5k5ccaf_set_preview_size(int size)
{
	int rc = 0;
#ifdef CAMERA_ESD_TEST_CODE
	pre_size = size;  // back-up for re-configurating by ESD
#endif
...
#ifdef CAMERA_ESD_TEST_CODE
	if((s5k5ccaf_ctrl->started) && (b_esd_timeout_det==false))
#else
...
int wait_sensor_mode(int mode, int interval, int cnt)
{
	...
	do {
		...
		CAM_DEBUG("current sensor mode = %d (wait = %d)", sensor_mode, cnt);
		msleep(interval);
...
{
	CAM_DEBUG("s5k5ccaf_ctrl->settings.preview_size_idx = %d", s5k5ccaf_ctrl->settings.preview_size_idx);
	...
	if(s5k5ccaf_ctrl->dtp_mode == 1) {
		...
		if(s5k5ccaf_ctrl->op_mode == S5K5CCGX_MODE_CAPTURE) {	// return to preview mode after capture
			CAM_DEBUG("return to preview mode");
			...
			CAM_DEBUG("s5k5ccaf_ctrl->cam_mode = %d", s5k5ccaf_ctrl->cam_mode);
			if(s5k5ccaf_ctrl->cam_mode == S5K5CCGX_MOVIE_MODE || s5k5ccaf_ctrl->cam_mode == S5K5CCGX_MMS_MODE) {
				...
				if(s5k5ccaf_ctrl->settings.scene == S5K5CCGX_SCENE_OFF) {					
					s5k5ccaf_set_whitebalance(s5k5ccaf_ctrl->settings.wb);
					s5k5ccaf_set_effect(s5k5ccaf_ctrl->settings.effect);
					s5k5ccaf_set_brightness(s5k5ccaf_ctrl->settings.brightness);
					s5k5ccaf_set_af_mode(s5k5ccaf_ctrl->settings.focus_mode);
					...
					if(s5k5ccaf_ctrl->settings.scene == S5K5CCGX_SCENE_NIGHTSHOT) {
						msleep(500);
						CAM_DEBUG("additional delay 500ms (NIGHTSHOT)"); 
					} else if(s5k5ccaf_ctrl->settings.scene == S5K5CCGX_SCENE_FIREWORK)  {
						msleep(800);
						CAM_DEBUG("additional delay 800ms (FIREWORK)");	
					} else {
						msleep(200);
					}							
				}
...
void s5k5ccaf_set_capture(void)
{
	...
	if(cur_lux > 0xFFFE) {
		...
		if((s5k5ccaf_ctrl->settings.scene == S5K5CCGX_SCENE_NIGHTSHOT) ||(s5k5ccaf_ctrl->settings.scene == S5K5CCGX_SCENE_FIREWORK)) {
...
int s5k5ccaf_sensor_ext_config(void __user *arg)
{
	...
	switch(cfg_data.cmd) {
 	...
 	case EXT_CFG_ESD_RESET:
        CAM_DEBUG("[ESD] EXT_CFG_TEST_ESD\n");
        if(cfg_data.value_1 == 1)
        {
        /* // check ESD reg. every 100 frame
            int read_reg;

            m5mo_readb(0x00, 0x0E, &read_reg);
            CAM_DEBUG("[ESD] Read Reg.0x0E = %d", read_reg);
            if(read_reg == 0x01) {
                b_esd_detected = true;
            }
        */   
            CAM_DEBUG("[ESD] b_esd_detected : %d",b_esd_detected);
            cfg_data.value_2 = b_esd_detected;
            b_esd_detected = false;  // reset to normal            
        }
        else
        {
            b_esd_timeout_det = true;
            CAM_DEBUG("[ESD] POWER OFF START");
            gpio_set_value_cansleep(CAM_3M_RST, LOW);
            mdelay(3);
            cam_ldo_power_off();
            mdelay(10);
            CAM_DEBUG("[ESD] POWER OFF END");

            CAM_DEBUG("[ESD] POWER ON START ");
            cam_ldo_power_on("s5k5ccaf");
            mdelay(1);
            ...
            gpio_set_value_cansleep(CAM_3M_RST, HIGH);
            mdelay(10);
            CAM_DEBUG("[ESD] POWER ON END ");        

            s5k5ccaf_set_sensor_mode(SENSOR_PREVIEW_MODE);            
            s5k5ccaf_set_preview_size(pre_size);	            
        }
        break;
#else
	case EXT_CFG_ESD_RESET:
	...
	case EXT_CFG_SET_FLASH_MODE:
		CAM_DEBUG("flash mode = %d",cfg_data.value_1);
	...
	case EXT_CFG_SET_PREVIEW_SIZE:	
		CAM_DEBUG("set preview size index = %d", cfg_data.value_1);
		s5k5ccaf_ctrl->settings.preview_size_idx = cfg_data.value_1;	// just set index
		break;
	...
	case EXT_CFG_GET_STATUS:
		...
		cfg_data.value_3 = read_value1;
			
		printk("check current module status : %x\n", cfg_data.value_3);
		printk("PINON/OFF : %d\n", gpio_get_value(0));
		break;
...
int s5k5ccaf_sensor_init(const struct msm_camera_sensor_info *data)
{
	...
	s5k5ccaf_ctrl->settings.effect = S5K5CCGX_EFFECT_OFF;
	s5k5ccaf_ctrl->settings.wb = S5K5CCGX_WB_AUTO;
	s5k5ccaf_ctrl->settings.brightness = S5K5CCGX_BR_DEFAULT;
...
int s5k5ccaf_sensor_release(void)
{
	...
	printk("[S5K5CCAF] s5k5ccaf_sensor_release\n");
	
	printk("[S5K5CCAF] set lens position to low\n");
	S5K5CCAF_WRITE_LIST(s5k5ccaf_af_off);
	msleep(100);

	printk("[S5K5CCAF] set off flash\n");
