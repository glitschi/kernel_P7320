			...
			void __user *arg)
{
	...
	qcmd_resp = __msm_control(sync,
				  &ctrl_pmsm->ctrl_q,
				  qcmd, msecs_to_jiffies(5000));	// org timeout - 10000
...
static int msm_get_sensor_info(struct msm_sync *sync, void __user *arg)
{
	...
	if(!strcmp(sdata->sensor_name,"s5k5ccaf"))
		cam_dev_num = 0;
	else
		cam_dev_num = 1;
	...
	unsigned long arg)
{
	...
	case MSM_CAM_IOCTL_ERROR_CONFIG:
        ...
        rc = s5k5ccaf_sensor_esd_detected();
        rc = s5k5bafx_sensor_esd_detected();            
       	CDBG("get_pic:MSM_CAM_IOCTL_ERROR_CONFIG\n");
#else        
	...
	unsigned long arg)
{
	...
	case MSM_CAM_IOCTL_GET_CAMERA_INFO:
		...
		break;
#ifndef CONFIG_SENSOR_MC7
	case MSM_CAM_IOCTL_EXT_CONFIG:
		if(cam_dev_num) {
			rc = s5k5bafx_sensor_ext_config(argp);
		} else {
...
static int __msm_release(struct msm_sync *sync)
{
	...
	if (!sync->opencnt) {
		...
		if (sync->core_powered_on) {
			...
			sync->sctrl.s_release();	// origin 
			CDBG("%s, msm_camio_sensor_clk_off\n", __func__);
			msm_camio_sensor_clk_off(sync->pdev);
			cam_ldo_power_off();
			//sync->sctrl.s_release();	// have tur turn off MCLK before LDOs
