...
static int f_esd_detected=false;
static int f_esd_timeout_det=false;
#endif 
/*=============================================================
...
#ifdef CAMERA_ESD_TEST_CODE
	if((s5k5bafx_ctrl->started) && (f_esd_timeout_det==false)) {
#else	
...
#ifdef CAMERA_ESD_TEST_CODE
	f_esd_detected = false;    
    f_esd_timeout_det = false;
#endif
...
int s5k5bafx_sensor_ext_config(void __user *arg)
{
	...
	case EXT_CFG_ESD_RESET:
        ...
        { 
            CAM_DEBUG("[ESD] f_esd_detected : %d",f_esd_detected);
            cfg_data.value_2 = f_esd_detected;
            f_esd_detected = false;  // reset to normal   
        }
        else 
        {
            f_esd_timeout_det = true;        
            s5k5bafx_esd_reset();
        }       
#else
        s5k5bafx_esd_reset();
	...
	case EXT_CFG_SET_PREVIEW_SIZE:	
		...
		s5k5bafx_ctrl->settings.preview_size_idx = cfg_data.value_1;	// just set index
		break;
