...
#if 1//defined(CONFIG_MACH_P8_LTE) //kth_111007
...
static int voice_set_device(struct voice_data *v)
{
	...
	cvp_setdev_cmd.hdr.hdr_field = APR_HDR_FIELD(APR_MSG_TYPE_SEQ_CMD,
				APR_HDR_LEN(APR_HDR_SIZE), APR_PKT_VER);
#if defined(CONFIG_MACH_P8_LTE) //kth_111007
	 if (v->voc_path == VOC_PATH_PASSIVE) {
#endif
...
#if defined(CONFIG_MACH_P8_LTE) //kth_111007
// (40 37 39 loopback) do not apply the diamond solution
//if((v->dev_rx.dev_id != 40) && (v->dev_rx.dev_id != 37) && (v->dev_rx.dev_id != 39)) {
	/* BEGIN: VPCM */
