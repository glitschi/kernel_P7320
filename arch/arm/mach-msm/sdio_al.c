...
/* 
 * even though got bootloader_done event,
 * MDM can crash before normal boot done event from kickstart.
 * so we need to detect such case by triggering mdm err routine when sync pin drops 
 */
											...
											char const *func)
{
	...
	if (sdio_al_dev->signature != SDIO_AL_SIGNATURE) {
		pr_err(MODULE_NAME ": %s: Invalid signature\n", func);
		return -ENODEV;
	}
	if (!sdio_al_dev->card) {
			       ...
			       char const *func)
{
	...
	if (sdio_al_dev->signature != SDIO_AL_SIGNATURE) {
		pr_err(MODULE_NAME ": %s: Invalid device signature\n", func);
		return -ENODEV;
	}

	if (!sdio_al_dev->host) {
		pr_err(MODULE_NAME ": %s: NULL host\n", func);
		return -ENODEV;
	}
	...
	char const *func)
{
	...
	if (sdio_al_dev->state != CARD_INSERTED) {
		pr_err(MODULE_NAME ": %s: Invalid device state %d\n", func, sdio_al_dev->state);
				   ...
				   int is_vote_for_sleep)
{
	...
	if (is_vote_for_sleep) {
		if ( sdio_al_dev->is_ready == true ) {
			pr_debug(MODULE_NAME ": %s - sdio vote for Sleep\n", __func__);
...
static int read_mailbox(struct sdio_al_device *sdio_al_dev, int from_isr)
{
	...
	if ((rx_notify_bitmask == 0) && (tx_notify_bitmask == 0) &&
		...
		DATA_DEBUG(MODULE_NAME ":Notify bitmask for card %d "
				       "rx=0x%x, tx=0x%x.\n",
			sdio_al_dev->host->index, rx_notify_bitmask,
			   tx_notify_bitmask);
#endif			   
			   ...
			   u32 not_from_int, struct sdio_channel *ch)
{
	...
	if (!sdio_al_dev->card ||
		...
		pr_err(MODULE_NAME ": NULL card or wk_func\n");
		return -ENODEV;
	}
		...
		if (sdio_al_dev->is_ready) {
			...
			if (!sdio_al_claim_mutex_and_verify_dev(sdio_al_dev,
				...
				if (!sdio_al_dev->card ||
					...
					pr_err(MODULE_NAME ": %s: Invalid func1", __func__);
					return;
				}
				func1 = sdio_al_dev->card->sdio_func[0];
...
static int channel_close(struct sdio_channel *ch, int flush_flag)
{
	...
	if (ch->state != SDIO_CHANNEL_STATE_OPEN) {
		pr_err(MODULE_NAME ":%s: ch %s is not in open state (%d)\n",
	...
	do {
		...
		ret = read_mailbox(sdio_al_dev, false);
		if (ret) {
			pr_err(MODULE_NAME ":%s: failed to read mailbox", __func__);
			goto error_exit;
		}
			sdio_al_release_mutex(ch->sdio_al_dev, __func__);
	} while (ch->read_avail > 0);
	...
	ch->state = SDIO_CHANNEL_STATE_CLOSED;
	pr_info(MODULE_NAME ":%s: Ch %s closed "
		"successfully\n", __func__, ch->name);
...
static void sdio_al_close_all_channels(struct sdio_al_device *sdio_al_dev)
{
	...
	pr_info(MODULE_NAME ": %s", __func__);

	if (!sdio_al_dev) {
		pr_err(MODULE_NAME ": %s: NULL device", __func__);
		return;
	}
	for (j = 0; j < SDIO_AL_MAX_CHANNELS; j++) {
		...
		if (ch->state == SDIO_CHANNEL_STATE_OPEN) {
			pr_err(MODULE_NAME ": %s: Call to sdio_close() for"
				" ch %s\n", __func__, ch->name);
			ret = channel_close(ch, false);
			if (ret) {
				pr_err(MODULE_NAME ": %s: failed sdio_close()"
							...
							*sdio_al_dev)
{
	...
	if (!sdio_al_dev->card || !sdio_al_dev->card->sdio_func[0]) {
		pr_err(MODULE_NAME ": %s: NULL func1 for card %d", __func__,
			sdio_al_dev->host->index);
		goto exit_err;
	}
	...
	pr_info(MODULE_NAME ": %s: Notifying SDIO "
						    "clients for card %d",
			__func__, sdio_al_dev->host->index);
	for (j = 0; j < SDIO_AL_MAX_CHANNELS; j++) {
		if (!pdev_arr[j])
	...
	pr_info(MODULE_NAME ": %s: Finished Notifying "
...
static void sdio_al_reset(void)
{
	...
	pr_info(MODULE_NAME ": %s completed", __func__);
}
#endif
...
static void msm_sdio_al_shutdown(struct platform_device *pdev)
{
	...
	pr_info(MODULE_NAME "Initiating msm_sdio_al_shutdown...");
	...
	pr_info(MODULE_NAME ": %s: msm_sdio_al_shutdown complete.", __func__);
}
...
static int sdio_al_client_setup(struct sdio_al_device *sdio_al_dev)
{
	...
	if (!sdio_al_dev->card || !sdio_al_dev->card->sdio_func[0]) {
		pr_err(MODULE_NAME ":NULL card or "
							       "func1\n");
		sdio_al_release_mutex(sdio_al_dev, __func__);
		return -ENODEV;
	}
						...
						const struct sdio_device_id *sdio_dev_id)				
{
	...
	if (!sdio_al_dev->rx_flush_buf) {
		...
		if (sdio_al_dev->rx_flush_buf == NULL) {
			pr_err(MODULE_NAME ":Fail to allocate rx_flush_buf for card %d\n",
				card->host->index);
			return -ENOMEM;
		}
	}
...
static void sdio_al_sdio_remove(struct sdio_func *func)
{
	...
	pr_info(MODULE_NAME ": %s: Finished Notifying SDIO clients for card %d",
			__func__, sdio_al_dev->host->index);
...
#else // defined(CONFIG_TARGET_LOCALE_KOR_SKT)
...
#endif // defined(CONFIG_TARGET_LOCALE_KOR_SKT)
