...
#define DEBUG_TIMER_BASE (MSM_TMR_BASE + 0x4)
#elif CONFIG_MSM7X00A_USE_DG_TIMER
#define DEBUG_TIMER_BASE (MSM_TMR_BASE + 0x24)
#endif
#endif
...
#if defined(CONFIG_TARGET_LOCALE_EUR_OPEN)
	char dmsg[16]={0};
	unsigned int timer_enable,timer_match_val;
#endif

	msm_pm_config_hw_before_swfi();

#if defined(CONFIG_TARGET_LOCALE_EUR_OPEN)
	timer_enable = __raw_readl(DEBUG_TIMER_BASE + 0x8);
	timer_match_val = __raw_readl(DEBUG_TIMER_BASE + 0x0);

	snprintf(dmsg, sizeof(dmsg), "TE:%x", timer_enable);
	sec_debug_task_sched_log_short_msg(dmsg);
	snprintf(dmsg, sizeof(dmsg), "TMV:%x", timer_match_val);
	sec_debug_task_sched_log_short_msg(dmsg);
#endif
