...
#endif
#ifdef CONFIG_SEC_DEBUG_HARDIRQS_LOG
	struct hardirqs_log gExcpHardIRQsLog[2][HARDIRQS_LOG_MAX] ;
#endif
...
extern void charm_assert_panic();
...
// P5 KOR : sh1.back 11. 11. 21  - add commercial dump fuctionality
#if defined(CONFIG_TARGET_LOCALE_KOR_SKT) || defined(CONFIG_TARGET_LOCALE_KOR_KT) || defined(CONFIG_TARGET_LOCALE_KOR_LGU) 	
		if( check_jig_on() & (!strcmp(buf, "Commercial Dump")) ) {
	...
	else if (!strcmp(buf, "MDM Crash"))
		sec_debug_set_upload_cause(UPLOAD_CAUSE_MDM_ERROR_FATAL);
	else if (!strcmp(buf, "LPASS Crash"))
...
#define LOCKUP_SECOND_KEY KEY_END
#define LOCKUP_THIRD_KEY KEY_END
...
// P5 KOR : sh1.back 11. 11. 21  - add commercial dump fuctionality
	...
	if (!enable) 
	{
    		...
    		case STEP10:
				...
				{
					pr_info("Enter commercial panic mode.+\n");
					if( check_jig_on() ) 
					{	
						pr_info("Enter commercial panic mode.\n");
...
__init int sec_debug_init(void)
{
	...
	if((sec_dbg_buf_ptr == NULL) || (sizeof(struct sec_debug_nocache) > sec_dbg_buf_size)) {
		...
		#endif
		#ifdef CONFIG_SEC_DEBUG_HARDIRQS_LOG
		sec_debug_nocache_log->gExcpHardIRQsLogIdx[0] = -1;
		sec_debug_nocache_log->gExcpHardIRQsLogIdx[1] = -1;
		#endif
	}
#endif
...
#ifdef CONFIG_SEC_DEBUG_HARDIRQS_LOG
void sec_debug_hardirqs_log(int irqoff, void* caller0, void* caller1)
{
	if (!enable) return;
	...
	if (sec_debug_nocache_log) {
		int cpu = smp_processor_id();
		unsigned i;
		i = (sec_debug_nocache_log->gExcpHardIRQsLogIdx[cpu]++) & (HARDIRQS_LOG_MAX - 1);
		sec_debug_nocache_log->gExcpHardIRQsLog[cpu][i].time = cpu_clock(cpu);
		sec_debug_nocache_log->gExcpHardIRQsLog[cpu][i].irqoff = irqoff;
		sec_debug_nocache_log->gExcpHardIRQsLog[cpu][i].caller0 = caller0;
		sec_debug_nocache_log->gExcpHardIRQsLog[cpu][i].caller1 = caller1;
		sec_debug_nocache_log->gExcpHardIRQsLog[cpu][i].context = &cpu;
	}
}
#endif
