...
#include "devices.h"
#include "clock.h"
#ifdef CONFIG_TARGET_LOCALE_EUR_OPEN
...
// Total timeout during shutdown is reduced to 5 sec which reduces total shutdown
...
#define ESD_CHECK_PERIOD	50

static void (*power_on_charm)(void);
static void (*power_down_charm)(void);

#ifdef CONFIG_TARGET_LOCALE_EUR_OPEN
#define ESD_GPIO_CHECK_INTERVAL	200

static struct timer_list esd_fatal_gpio_timer;
static struct timer_list esd_status_gpio_timer;

static int esd_fatal_gpio_check_cnt=0;
static int esd_status_gpio_check_cnt=0;
#endif

static int charm_debug_on = 1; // default enable
static int charm_status_irq;
static int charm_errfatal_irq;
int charm_ready = 0;
EXPORT_SYMBOL(charm_ready);
...
#ifdef CONFIG_SEC_DEBUG
void charm_assert_panic()
{
	CHARM_DBG("%s: setting AP2MDM_ERRFATAL high\n", __func__);
	gpio_set_value(AP2MDM_ERRFATAL, 1);
	gpio_set_value(AP2MDM_WAKEUP, 1); // wake up mdm to get ap2mdm errfatal correctly
	mdelay(20);
}
#endif
...
extern int firmware_downloading;
#endif

static void charm_status_fn(struct work_struct *work)
{
#ifdef CONFIG_TOUCHSCREEN_MELFAS
	if (!firmware_downloading){	//TSP firmware download
#endif	
		...
		if ((gpio_get_value(MDM2AP_STATUS) == 0) && charm_ready) {			
			...
			pr_info("charm was not shut down actually...return func\n");
		}
#ifdef CONFIG_TOUCHSCREEN_MELFAS
	}
#endif
...
#ifdef CONFIG_TARGET_LOCALE_EUR_OPEN
static void esd_fatal_gpio_timer_func(unsigned long data)
{
	if (gpio_get_value(MDM2AP_ERRFATAL) == 1)
	{
		if ( esd_fatal_gpio_check_cnt >= 5 )
		{
			if (charm_ready && (gpio_get_value(MDM2AP_STATUS) == 1)) {
				CHARM_DBG("%s: scheduling work now\n", __func__);
				queue_work(charm_queue, &charm_fatal_work);
			}
			esd_fatal_gpio_check_cnt=0;
		}
		else
		{
			esd_fatal_gpio_check_cnt++;
			mod_timer(&esd_fatal_gpio_timer, jiffies + msecs_to_jiffies(ESD_GPIO_CHECK_INTERVAL));
		}
	}
	else
	{
		CHARM_DBG( "esd_fatal_gpio_timer_func called but ignored");
		esd_fatal_gpio_check_cnt=0;
		del_timer(&esd_fatal_gpio_timer);
	}
}

static void esd_status_gpio_timer_func(unsigned long data)
{
	if (gpio_get_value(MDM2AP_STATUS) == 0)
	{
		if ( esd_status_gpio_check_cnt >= 5 )
		{
			if ((gpio_get_value(MDM2AP_STATUS) == 0) && charm_ready) {
				CHARM_DBG("%s: scheduling work now\n", __func__);
			...
			esd_status_gpio_check_cnt=0;
		}
		else
		{
			esd_status_gpio_check_cnt++;
			mod_timer(&esd_status_gpio_timer, jiffies + msecs_to_jiffies(ESD_GPIO_CHECK_INTERVAL));
		}
	}
	else
	{
		CHARM_DBG( "esd_status_gpio_timer_func called but ignored");
		esd_status_gpio_check_cnt=0;
		del_timer(&esd_status_gpio_timer);
	}
}
#endif
...
static irqreturn_t charm_status_change(int irq, void *dev_id)
{
	...
	if( desc->irq_count > 50 && (printk_rate++ % 100) < 3 ) {
		printk("## NK [%s:%d:%d] 0x%08x, 0x%08x, 0x%08x \n", __func__ , __LINE__ , smp_processor_id(),
			readl(GPIO_IN_OUT(MDM2AP_STATUS)), readl(GPIO_INTR_CFG(MDM2AP_STATUS)), readl(GPIO_INTR_STATUS(MDM2AP_STATUS)));
	}
...
static void charm_modem_shutdown(struct platform_device *pdev)
{
	...
	if ((i <= 0)||charm_shutdown) {
		if ( charm_shutdown )
			pr_err("%s: MDM2AP_STATUS went low. but we still need to force power down mdm pmic\n",__func__);
		else
			pr_err("%s: MDM2AP_STATUS never went low.\n",__func__);
	}
		gpio_direction_output(AP2MDM_PMIC_RESET_N, 1);
...
{
#ifdef CONFIG_TARGET_LOCALE_EUR_OPEN
	init_timer(&esd_fatal_gpio_timer);
	esd_fatal_gpio_timer.function = esd_fatal_gpio_timer_func;
	esd_fatal_gpio_timer.data = (unsigned long)NULL;

	init_timer(&esd_status_gpio_timer);
	esd_status_gpio_timer.function = esd_status_gpio_timer_func;
	esd_status_gpio_timer.data = (unsigned long)NULL;
#endif
