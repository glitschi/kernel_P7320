...
#define BARK_TIME_SEC 20
#define BITE_TIME_SEC 22
...
#ifdef WDOG_PET_IN_TIMER
static struct timer_list dogpet_timer;
#else
...
static int msm_watchdog_suspend(void)
{
	...
	dsb();
	pr_info("MSM Watchdog Suspended!\n");
	return NOTIFY_DONE;
}
static int msm_watchdog_resume(void)
{
	pr_info("MSM Watchdog Resume!\n");
...
{
	int wdt_cnt;
	static int last_wdt_cnt = 0;
	
	wdt_cnt = (*(unsigned int*)WDT0_STS)>>3;
	__raw_writel(1, WDT0_RST);
	pr_info("%s: wdt_cnt=%x\n", __func__, wdt_cnt);
	__raw_writel(1, WDT0_RST);
	
	if(last_wdt_cnt && (wdt_cnt > (last_wdt_cnt<<1))) {
		pr_crit("%s: wdog counter was not reset!\n", __func__);
		__raw_writel(1, WDT0_RST);
		__raw_writel(0, WDT0_RST);
		__raw_writel(1, WDT0_RST);
	}
	
	last_wdt_cnt = wdt_cnt;
	last_pet = sched_clock();
}

#ifdef WDOG_PET_IN_TIMER
static void dogpet_timer_func(unsigned long data)
{
	pet_watchdog();
	...
	if (enable)
		mod_timer(&dogpet_timer, jiffies + delay_time);
}
#else
static void pet_watchdog_work(struct work_struct *work)
{
...
static int __init init_watchdog(void)
{
	...
	__raw_writel(32768*BARK_TIME_SEC, WDT0_BARK_TIME);
	__raw_writel(32768*BITE_TIME_SEC, WDT0_BITE_TIME);
...
#ifdef WDOG_PET_IN_TIMER
	init_timer(&dogpet_timer);
	dogpet_timer.function = dogpet_timer_func;
	dogpet_timer.data = (unsigned long)NULL;
	dogpet_timer.expires = jiffies + delay_time;
	add_timer(&dogpet_timer);
#else
	schedule_delayed_work(&dogwork_struct, delay_time);
#endif
